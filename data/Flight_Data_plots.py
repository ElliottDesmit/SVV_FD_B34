...
motion = dutch_roll # choose motion # USER INPUT
...
t = DATA[indexes,48].astype(float)
...
peaks = where(abs(hstack((0,value))[:-1]-value) < 0.05)
peaks = peaks[0][where(value[peaks] > average(value[peaks]))]
xp = t[peaks]
fp = value[peaks]
a,b,c = polyfit(xp,fp,2)
y = a*t**2 + b*t + c

a0 = y[0]
i_half = where(abs(y-a0/2) == min(abs(y-a0/2)))
...
ax2.plot(t,DATA[dom,y_ax2].astype(float),color=col2)
...
#subplot 2
ax3.plot(t,DATA[dom,y_ax3].astype(float),color=col1)
...
ax4.plot(t,DATA[dom,y_ax4].astype(float),color=col2)
